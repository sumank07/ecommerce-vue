  ...
  <header class="sticky top-0 z-50 transition-all duration-300">
    ...
    >
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <!--Logo + Mobile Menu Touggle-->
          <div class="flex justify-between items-center w-full md:w-auto">
            <a href="/" class="text-2xl font-bold text-black-600">OurShop</a>
            <button
              class="md:hidden text-gray-700 hover:text-indigo-600"
              aria-label="Toggle moble menu"
          ...
          <!-- Search Bar
      <form class="w-full md:flex max-w-sm" role="search" aria-label="site search">
      <label class="relative w-full">
      <input type="text" placeholder="Search..." class="w-full px-3 py-2 text-xs border border-gray-300 rounded:full 
      focus:outline-none focus:ring-2 focus:ring-indigo-500">
      <button type="submit" class="absolute right-2 top-1/2 translate-y-1/2 text-gray-500 hover:text-indigo-600"
      ...
      <Icon icon="cart-outline" class="w-5 h-5"/>
      ...
      flex items-center justify-center">3</span>
       ...
       <button class="relative p-2 text-gray-700 hover:text-pink-600 " aria-label="wishlist">
      <Icon icon="account-outline" class="w-5 h-5"/>
      </button>
      </aside> -->
          <!-- Search Bar -->
          <form
            class="w-full md:flex max-w-sm"
            role="search"
    ...
    <nav class="bg-pink-950" aria-label="Main navigation">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg-px-8">
        ...
        <!-- <section v-if="isMobileMenuOpen" class="md:hidden mt-2 rounded-lg shadow-md p-4 space-y-3 text-[#5D4037] text-center" -->
        <section
          v-if="isMobileMenuOpen"
          class="md:hidden mt-2 rounded-lg shadow-md p-4 space-y-3 text-[#f4f0ef] text-center"
